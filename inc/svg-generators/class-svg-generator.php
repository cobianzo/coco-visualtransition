...
class SVG_Generator {
	 ...
	 */
	public function __construct( string $pattern_name = '', string $id = 'mi-greca', $atts = [] ) {
	...
	public function generate_points(): string {
		if ( ! empty( $this->pattern_name ) ) {
			...
			if ( ! empty( $this->pattern_data['patternRepeat'] ) && 'repeat-x' === $this->pattern_data['patternRepeat'] ) {
				...
				$offset_x            = 0.1;
				$offset_y            = 0.1;
				$this->points_string = self::generate_points_string_from_pattern( $this->pattern_data['pattern'], $this->number_figures, $this->pattern_height, $offset_x, $offset_y );
			}
		}
...
SVG;
	 ...
	 * @param integer $number_figures The number of times to repeat the pattern.
